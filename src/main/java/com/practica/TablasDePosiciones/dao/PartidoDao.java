...
public interface PartidoDao extends CrudRepository<Partido, Integer> {
	...
	@Query("SELECT p FROM Partido p WHERE (p.local = ?1 OR p.visitante = ?1) AND p.fecha.torneo = ?2 AND p.estado = ?3")
	List<Partido> findByEquipoAndTorneoAndEstado(Equipo equipo, Torneo torneo, EstadoPartido estado);
	...
	public List<Partido> findByFechaAndEstado(Fecha fecha, EstadoPartido estado);
}
